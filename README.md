...
Deformable DETR and RT-DETR predict a small set of **sampling locations** (typically 4 points × 4 scales × 8 heads = 128 total) per query. Each head looks only at these sparse positions — a key innovation that makes attention efficient for high-resolution feature maps.
...
This architectural difference is why the **Sampling Offsets** visualization only works for Deformable DETR and RT-DETR. For DAB-DETR, spatial heatmaps are produced by simply reshaping the flat `seq_len` attention dimension back to `(H_feat, W_feat)` — no Gaussian splatting needed.
...
Cross-attention weights reshaped directly from `(25×34)` feature-map resolution to image size via bilinear interpolation — no splatting needed.
